    ...
    def test_tdf105351_cond_format_data_bar(self):
        with self.ui_test.create_doc_in_start_center("calc"):
            ...
            with self.ui_test.execute_modeless_dialog_through_command(".uno:DataBarFormatDialog") as xDialog:
                ...
                #4. In the conditional formatting window, select the dropdown on the left that says "自动"
                #and change it to "值". In the text field below it enter 0.
                #5. Select the dropdown on the right that says "自动" and change it to "值". In the text field below it enter 10
                select_by_text(xcolscalemin, "值")
                xedcolscalemin.executeAction("TYPE", mkPropertyValues({"TEXT":"0"}))

                select_by_text(xcolscalemax, "值")
            ...
            with self.ui_test.execute_modeless_dialog_through_command(".uno:DataBarFormatDialog") as xDialog:
                ...
                self.assertEqual(get_state_as_dict(xcolscalemin)["SelectEntryText"], "值")
                self.assertEqual(get_state_as_dict(xedcolscalemin)["Text"], "0")
                self.assertEqual(get_state_as_dict(xcolscalemax)["SelectEntryText"], "值")
