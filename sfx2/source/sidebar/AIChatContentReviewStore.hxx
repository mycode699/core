...
struct AIChatContentReviewEntry
{
    OUString ReviewId;
...
struct AIChatContentReviewCreateResult
{
    bool Success = false;
    AIChatContentReviewEntry Review;
    AIChatContentRegistryEntry RegistryEntry;
    OUString Message;
};

class AIChatContentReviewStore final
{
public:
    AIChatContentReviewStore();

    const OUString& GetReviewStoreUrl() const { return m_sReviewStoreUrl; }

    AIChatContentReviewCreateResult
    CreateReviewFromSource(const AIChatContentRegistryEntry& rSource) const;
    std::vector<AIChatContentReviewEntry> LoadEntries() const;
