...
namespace
{
constexpr OUStringLiteral FORMATTING_REVIEW_DIR_NAME = u"kqoffice-v3-ai-formatting-review";
constexpr OUStringLiteral FORMATTING_REVIEW_FILE_NAME = u"formatting-reviews.tsv";
constexpr sal_uInt64 MAX_FORMATTING_REVIEW_STORE_BYTES = 1024 * 1024;
...
bool ReadUtf8File(const OUString& rUrl, OString& rContent)
{
    ...
    if (aFile.getSize(nSize) != osl::FileBase::E_None
        || nSize > MAX_FORMATTING_REVIEW_STORE_BYTES)
    {
        aFile.close();
        return false;
    }
...
bool ParseReviewLine(const OUString& rLine, AIChatFormattingReviewEntry& rEntry)
{
    ...
    rEntry.FormattingScope = aFields[2];
...
AIChatFormattingReviewStore::AIChatFormattingReviewStore()
{
    ...
    m_sStorageRootUrl = EnsureNoTrailingSlash(sRoot) + u"/"_ustr + FORMATTING_REVIEW_DIR_NAME;
    osl::Directory::createPath(m_sStorageRootUrl);
    m_sReviewStoreUrl = m_sStorageRootUrl + u"/"_ustr + FORMATTING_REVIEW_FILE_NAME;
}

bool AIChatFormattingReviewStore::IsSupportedFormattingScope(const OUString& rScope)
{
    return rScope == u"paragraph-style"_ustr || rScope == u"character-style"_ustr
           || rScope == u"table-layout"_ustr || rScope == u"cell-format"_ustr
           || rScope == u"slide-layout"_ustr;
}

OUString AIChatFormattingReviewStore::MakeReviewId(const OUString& rSourceObjectId)
{
    return u"formatting-review:"_ustr + rSourceObjectId;
}

AIChatFormattingReviewCreateResult AIChatFormattingReviewStore::CreateReviewFromSource(
    const AIChatContentRegistryEntry& rSource) const
{
    AIChatFormattingReviewCreateResult aResult;

    if (rSource.ObjectId.isEmpty())
    {
        aResult.Message = u"formatting-review-create-failed reason=missing-source-object-id"_ustr;
        return aResult;
    }
    if (!IsSupportedFormattingScope(rSource.Type))
    {
        aResult.Message = u"formatting-review-create-failed reason=unsupported-formatting-scope scope="_ustr
                          + rSource.Type;
        return aResult;
    }
    ...
    {
        aResult.Message
            = u"formatting-review-create-failed reason=missing-evidence-link source-id="_ustr
              + rSource.ObjectId;
        return aResult;
    }
    ...
    {
        aResult.Message
            = u"formatting-review-create-failed reason=missing-hash-reference source-id="_ustr
              + rSource.ObjectId;
        return aResult;
    }
    ...
    aResult.Review.FormattingScope = rSource.Type;
    aResult.Review.State = u"queued"_ustr;
    aResult.Review.ReviewMode = u"before-after-layout-diff"_ustr;
    ...
    const OUString sLine
        = EscapeField(aResult.Review.ReviewId) + u"\t"_ustr
          ...
          + EscapeField(aResult.Review.FormattingScope) + u"\t"_ustr
    ...
    {
        aResult.Message
            = u"formatting-review-create-failed reason=store-write-failed source-id="_ustr
              + rSource.ObjectId;
        return aResult;
    }

    aResult.RegistryEntry.ObjectId = aResult.Review.ReviewId;
    aResult.RegistryEntry.Type = u"formatting-preview"_ustr;
    aResult.RegistryEntry.SourceSurface = u"formatting-review"_ustr;
    ...
    aSource.SourceType = u"formatting-preview"_ustr;
    ...
    aSource.SourceSurface = u"formatting-review"_ustr;
    aSource.OpenTarget = aResult.Review.OpenTarget;
    aSource.SpanReference = u"span:formatting-scope:"_ustr + rSource.ObjectId;
    aSource.ReviewId = aResult.Review.ReviewId;
    aProvenance.RegisterSource(aSource);

    aResult.Success = true;
    aResult.Message = u"formatting-review-created review-id="_ustr + aResult.Review.ReviewId
                      + u" source-id="_ustr + rSource.ObjectId + u" scope="_ustr
                      + aResult.Review.FormattingScope
                      + u" review-mode=before-after-layout-diff uses-diff-review=true"_ustr
...
std::vector<AIChatFormattingReviewEntry> AIChatFormattingReviewStore::LoadEntries() const
{
    OString sContent;
    ...
    std::vector<AIChatFormattingReviewEntry> aEntries;
    ...
    while (nIndex >= 0)
    {
        ...
        AIChatFormattingReviewEntry aEntry;
